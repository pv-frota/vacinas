...
abstract class SembastDatabase extends SembastTransactionHandler {
  ...
  Future<Map<String, dynamic>?> get(
      {required String id, required String store});

  /// Retrieves all objects within [store]
  Future<List<Map<String, dynamic>>> getAll(
      {required String store, sembast.Finder? finder});

  /// Retrieves all objects with the following [ids] from the [store]
  Future<List<Map<String, dynamic>?>> getAllByIds(
      {required List<String> ids, required String store});
  ...
  Future<Stream<Map<String, dynamic>?>> listenTo(
      {required String id, required String store});
}
...
class SembastDatabaseImpl extends SembastDatabase {
  ...
  Future<void> put({
    ...
    await storeMap
        .record(id)
        .put(currentTransaction ?? db, object, merge: shouldMerge);
  }
  ...
  Future<void> putAll({
    ...
    await storeMap
        .records(ids)
        .put(currentTransaction ?? db, objects, merge: shouldMerge);
  }
  ...
  Future<void> removeAll(
      {required List<String> ids, required String store}) async {
    final storeMap = sembast.stringMapStoreFactory.store(store);
  ...
  @override
  Future<List<Map<String, dynamic>>> getAll(
      {required String store, sembast.Finder? finder}) async {
    final storeMap = sembast.stringMapStoreFactory.store(store);

    final allRecords =
        await storeMap.find(currentTransaction ?? db, finder: finder);
  ...
  @override
  Future<Stream<List<Map<String, dynamic>>>> listenAll(
  ...
  @override
  Future<Stream<Map<String, dynamic>?>> listenTo(
      {required String id, required String store}) async {
    final storeMap = sembast.stringMapStoreFactory.store(store);
